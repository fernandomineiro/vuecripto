<template>
  <form autocomplete="off">
    ...
    <modal v-if="!alert && !tradeInvoice.show" id="boleta">
      ...
      <template slot="body">
        <md-button class="md-round md-dense" v-bind:class="{'md-default': !buying, 'md-info': buying}" @click="buying = true">{{ $t("buy") }}</md-button>
        <md-button class="md-round md-dense" v-bind:class="{'md-default': buying, 'md-info': !buying}" @click="buying = false">{{ $t("sell") }}</md-button>
        <md-content class="md-layout" style="margin-top: 20px">
          <div v-if="!buying" class="md-layout-item md-caption">
          ...
          <div v-else class="md-layout-item md-caption">
            {{ $t("balance", { cur: quoteSymbol }) }}:
            {{ formatCurrencyValue(quoteBalance, data.currencyQuote.id) }}
            <h4 class="card-title">
              {{ $t("buy") }}
              <currency-symbol :currency="data.currency_base" />
            </h4>
...
export default {
  ...
  data() {
    return {
      ...
      buy_amount: null,
      sell_amount: null,
      selectedColor: 'md-accent',
      chipColor: "md-primary",
      alert: false,
      tradeInvoice: {
        data: null,
        show: false
